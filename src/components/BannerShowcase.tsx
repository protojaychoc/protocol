...
export const BannerShowcase = ({ bannerShowcase }: BannerShowcaseProps) => {
  return (
    <section className="relative desktop:mt-36 block mx-auto max-w-theme-wide">
      <div className="desktop:absolute z-10 desktop:-top-24 desktop:left-[calc(13rem)] desktop:right-[calc(13rem)] min-h-36 flex flex-col desktop:flex-row desktop:justify-start desktop:items-center items-start desktop:bg-card text-card-foreground gap-4 px-2 sm:px-6 desktop:px-0 rounded-xl">
        {bannerShowcase.bannerShowcase.map((showcase, index) => (
          <Link
            key={index}
            href=""
          ...
          >
            <div className="desktop:flex desktop:flex-col desktop:justify-between h-full desktop:min-h-[300px]">
              <div>
                <span
                  className="block mx-auto w-[40px]"
      ...
      <picture className="relative min-h-[600px] -top-24 desktop:mb-0 desktop:static desktop:top-0 desktop:z-0 -z-10 block w-full bg-black rounded-xl overflow-hidden">
        <Image
          src={bannerShowcase.img}
