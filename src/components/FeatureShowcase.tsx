  ...
  return (
    <section className="mt-28 mb-40 lg:mt-36 lg:mb-0 block mx-auto max-w-theme-wide">
      <div className="grid grid-cols-1 lg:grid-cols-2 desktop:grid-cols-3 gap-x-4 gap-y-20 desktop:gap-y-36">
        {featureShowcase.features.map(
          (feature: FeatureShowcaseProps, index: number) => (
            <Link
              key={index}
              href=""
              className="relative max-w-[549px] block group mx-auto"
            >
              <Image
                src={feature.image}
                width={549}
                height={381}
                alt={feature.title}
                className="rounded-lg group-hover:shadow-md object-cover w-auto h-full"
              ></Image>
