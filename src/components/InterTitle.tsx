...
export const InterTitle = ({ interTitle }: InterTitleProps) => {
  const highlight = interTitle.highlight || "";
  const interTitleParts = interTitle?.title.split(highlight);

  return (
    <section
      className={`block mx-auto max-w-theme-wide desktop:px-52 ${
        interTitle.mt ? "lg:mt-96 mt-24" : "lg:mt-72 mt-24"
      }`}
    >
      <div
        className={`flex flex-col ${
          interTitle.hLvh ? "h-lvh" : ""
      ...
      >
        <div className="text-left lg:text-center max-w-[900px] m-auto block">
          ...
          {interTitle.subtitle && (
            <h3 className="text-md lg:text-2xl mt-5 my-10">
              {interTitle.subtitle}
            </h3>
          )}
        </div>
